...
inline frc::Pose3d toPose3D(const std::vector<double> &inData) {
  ...
  return frc::Pose3d(frc::Translation3d(units::length::meter_t(inData[0]),
...
inline void SetFiducialDownscaling(const std::string &limelightName,
                                   float downscale) {
  int d = 0;  // pipeline
  if (downscale == 1.0) {
    d = 1;
...
inline double extractArrayEntry(const std::vector<double> &inData,
                                int position) {
...
inline std::vector<RawFiducial> getRawFiducials(
  ...
  nt::NetworkTableEntry entry =
      LimelightHelpers::getLimelightNTTableEntry(limelightName, "rawfiducials");
...
class RawDetection {
 public:
  int classId{-1};
  double txnc{0.0};
  double tync{9.0};  // It seems like you intentionally set this to 9.0, so I
                     // kept it as is.
  double ta{0.0};
...
inline std::vector<RawDetection> getRawDetections(
    const std::string &limelightName) {
  nt::NetworkTableEntry entry = LimelightHelpers::getLimelightNTTableEntry(
      limelightName, "rawdetections");
  ...
  for (int i = 0; i < numDetections; ++i) {
    int baseIndex = i * valsPerEntry;
    int classId =
        static_cast<int>(extractArrayEntry(rawDetectionArray, baseIndex));
...
inline std::optional<PoseEstimate> getBotPoseEstimate(
    const std::string &limelightName, const std::string &entryName) {
...
inline std::optional<PoseEstimate> getBotPoseEstimate_wpiBlue(
    const std::string &limelightName = "") {
  return getBotPoseEstimate(limelightName, "botpose_wpiblue");
}

inline std::optional<PoseEstimate> getBotPoseEstimate_wpiRed(
    const std::string &limelightName = "") {
  return getBotPoseEstimate(limelightName, "botpose_wpired");
}

inline std::optional<PoseEstimate> getBotPoseEstimate_wpiBlue_MegaTag2(
    const std::string &limelightName = "") {
  return getBotPoseEstimate(limelightName, "botpose_orb_wpiblue");
}

inline std::optional<PoseEstimate> getBotPoseEstimate_wpiRed_MegaTag2(
    const std::string &limelightName = "") {
...
//         sendto(sockfd, msg, strlen(msg), 0, (const struct sockaddr *)
...
//     ssize_t n = recvfrom(sockfd, (char *)receiveData, 1024, 0, (struct
//     sockaddr *) &cliaddr, &len); if (n > 0) {
...
inline void from_json(const wpi::json &j, VisionResultsClass &t) {
  ...
  t.ClassificationResults =
      SafeJSONAccess<std::vector<ClassificationResultClass>>(
          j, "Detector", std::vector<ClassificationResultClass>{});
}

inline void from_json(const wpi::json &j, LimelightResultsClass &t) {
  t.targetingResults = SafeJSONAccess<LimelightHelpers::VisionResultsClass>(
